...
class MusicManager
{
	...
	{
		$queryString = "SELECT owns.song_id, owns.title, owns.artist, song.location, owns.rating FROM owns INNER JOIN song ON owns.song_id=song.song_id WHERE owns.user_id=? AND owns.title LIKE ? AND owns.artist LIKE ? ORDER BY artist, title";
	...
	public function likeSong($song_id, $owner_id)
	{
		...
		$stmt = "INSERT INTO likes(user_id, song_id, owner_id, like_value) VALUES (?,?,?,1)";
	...
	public function dislikeSong($song_id, $owner_id)
	{
		...
		$stmt = "INSERT INTO likes(user_id, song_id, owner_id, like_value) VALUES (?,?,?,0)";
		$query = $this->database->prepare($stmt);
		$query->bind_param("iii", $this->user_id, $song_id, $owner_id);
